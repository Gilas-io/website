...
|---|---|---|---|
| `claude-3-7-sonnet-latest` | Anthropic | جدیدترین و قدرتمندترین مدل از خانواده‌ Claude که قابلیت استدلال و تفکر زنجیره‌ای برای تحلیل عمیق مسایل و راه‌حل های احتمالی را دارد. این مدل از تمام قابلیت های مدل Claude 3.5 Sonnet برخوردار است. | ۲۰۰.۰۰۰ توکن |
| `claude-3-5-sonnet-latest` | Anthropic | مدلی قدرتمند با توانایی‌های پیشرفته در استدلال، کدنویسی و پردازش تصویر. این مدل با سرعت دو برابر نسبت به Claude 3 Opus عمل می‌کند و هزینه کمتری دارد. مناسب برای پشتیبانی مشتری و انجام وظایف چندمرحله‌ای. این مدل قابلیت درک تصاویر را نیز دارد. | ۲۰۰.۰۰۰ توکن |
...
`gpt-4o-mini-search` | OpenAI | این مدل برای جستجوی اطلاعات در اینترنت طراحی شده است و از طریق اندپوینت `/v1/chat/completions` قابل دسترسی است. | ۱۲۸.۰۰۰ توکن
`gpt-4o-search` | OpenAI | این مدل برای جستجوی اطلاعات در اینترنت طراحی شده است و از طریق اندپوینت `/v1/chat/completions` قابل دسترسی است. | ۱۲۸.۰۰۰ توکن
...
### مدل‌های تبدیل صوت به متن

 مدل | ارایه دهنده | توضیحات  | 
---|---|---|
`whisper-1` | OpenAI | ویسپر، یک مدل عمومی برای تشخیص صداست که با استفاده از داده‌های صوتی گسترده و متنوعی آموزش داده شده است. علاوه بر این، می‌توان از آن به عنوان یک مدل چندکاره برای تشخیص گفتار به زبان‌های مختلف، ترجمه گفتار و شناسایی زبان نیز بهره برد  |
`gpt-4o-mini-transcribe` | OpenAI | این مدل نسبت به مدل‌های اصلی Whisper، نرخ خطای کلمات را کاهش داده و دقت و تشخیص زبان را بهبود بخشیده است. |
`gpt-4o-transcribe` | OpenAI | این مدل نسبت به مدل‌های اصلی Whisper، نرخ خطای کلمات را کاهش داده و دقت و تشخیص زبان را بهبود بخشیده است. |

### مدل‌های تبدیل متن به صوت  

 مدل | ارایه دهنده | توضیحات  | 
---|---|---|
`tts-1` | OpenAI | مدل `tts-1` برای تبدیل متن به گفتار آنی و بی‌درنگ طراحی شده است  |
`tts-1-hd` | OpenAI | مدل `tts-1-hd` برای کاربردهای تبدیل متن به گفتار با کیفیت بالا بهینه شده است. |
`gpt-4o-mini-tts-1` | OpenAI | این مدل برای تبدیل متن به گفتار است که بر پایه  `GPT-4o-Mini`، یک مدل زبانی سریع و قدرتمند، ساخته شده است.  این مدل با کیفیت‌ ترین صوت را برای متون فارسی تولید می کند.|
